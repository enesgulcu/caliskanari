...
const ImageUploader = ({fileLocation, fileName}:{fileLocation:string, fileName:string}) => {
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [selectedImage, setSelectedImage] = useState<File | null>(null);
  const [buttonColor, setButtonColor] = useState<string>('#313131');
  const [buttonText, setButtonText] = useState<string>('Resim Yükle');
  ...
  const handleUpload = () => {
    if (selectedImage) {
      ...
      reader.onloadend = () => {
        ...
        console.log('Resim yüklendi:', imageData);
      };
